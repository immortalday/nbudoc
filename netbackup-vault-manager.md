...
7. После того как профиль будет создан, будет отображено окно редактирования параметров профиля.
  ...
  **Параметр Location**
  ...
  | Параметр | Описание |
  | --- | --- |
  | **Skip the Duplication step** | Определяет, что данный шаг не будет выполнен в профиле |
  | **Source backups reside on** | Определяет расположение образов резервных копий. Диск или ленточный носитеиль и(или) оба типа. **Disk Only** - определяет что только на дисковом хранилище. **Removable media and/or disk** - определяет что диск или ленточный носитель и(или) оба типа. **Nubmer of read drives** - определяет количество приводов, которые могут быть использованы для чтения образов резервных копий. |
