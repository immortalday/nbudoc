...
7. После того как профиль будет создан, вы попадаете в окно редактирования параметров профиля.
  ...
  **Параметр Location**
  ...
  | --- | --- |
  | basic disks | Определяет (явно) место расположения образов резерных копий (для типа хранилища basic disk) для профиля Vault|
  | Disk Pools | Определяет (явно) место расположения образов резервных копий (для типа хранилища Disk Pool) для профиля Vault |
  | Source volume groups | Определяет (явно) место расположения образов резервных копий |
  | Volume Pools || 
